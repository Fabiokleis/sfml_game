...
void Level2::arbritary_positions() {
    ...
    for (int u = 0; u < this->walls_number; u++) {
        ...
        int distrSpaceW = generate_random(128, (plat_sx - 32)); // the local of wall
    ...
    for (int pp = 0; pp < this->platforms_number; pp++) {
        for (int c = 0; c < prop; c++) {
            ...
            this->coins[c_].set_position(aux_c.get_position().x + generate_random(256, 512), this->height - plat_sy - generate_random(160, 192));
            aux_c = this->coins[c_];
            c_++;
        }
    }
